...
shinyServer(function(input, output) {
  ...
  limite_cores_y <- 1.05 * max(c(abs(aux_eixos_fixos$Cresci_Acum_12M), abs(base_pib_aux$PIB_Y)), na.rm = T)
    ...
    base_aux %>%
      plot_ly(x = ~Variavel_X, 
              ...
              mode = 'markers', 
              color = ~NomeVar,
              marker = list(size = 13, # Controla o tamanho do marker
                            sizeref = .15, 
                            color = "#004B82",# color = "#004B82" color = ifelse(base_aux$CategoriaPIB == "Coincidente", "#004B82", ifelse(base_aux$CategoriaPIB == "Antecedente", "#FF0000", "#00FF00"))
														symbol = 1:length(base_aux$NomeVar) + 12),  
              hoverinfo = "text",
	...
	ano_data_tabela <- lubridate::year(ymd(input$data_tabela_variacoes))
	mes_data_tabela <- lubridate::month(ymd(input$data_tabela_variacoes))
	
	paste0("Tabela Sintética de Variações em ", retorna_mes(mes_data_tabela), " de ", ano_data_tabela)
	
	})
	
	output$tabela_variacoes_final <- renderFormattable ({
  ...
  series_contra_ci <- c("s_1005", "s_1006", "s_1016")
	...
	data_analise <- input$data_tabela_variacoes
	...
	tabela_variacoes_pos <- tabela_variacoes_pre %>%
	...
	colnames(tabela_variacoes_pos) <- c("Nome da Série", "Valor Bruto", "Mês contra mesmo mês do ano anterior*", "Média em 12 meses*", "Mês contra mês imediatamente anterior**", "Média Móvel 3 Meses**", "Cod Série", "Categoria")
	
	
	formattable(tabela_variacoes_pos, 
		align = c("l", "c", "r", "r", "r", "r"),
		list(
		...
		"Valor Bruto" = formatter("span",
						 ifelse(tabela_variacoes_pos$"Nome da Série" == "Agronegócio (pessoas ocupadas)", 
						 formatC(tabela_variacoes_pos$"Valor Bruto", digits = 0, big.mark = ".", format = "f", decimal.mark = ","), 
						 formatC(tabela_variacoes_pos$"Valor Bruto", digits = 1, big.mark = ".", format = "f", decimal.mark = ","))
						 ),

		"Mês contra mesmo mês do ano anterior*" = formatter("span",
                         style = x ~ style(color = ifelse(tabela_variacoes_pos$"Categoria" == "Índices de Preços", NA, ifelse(tabela_variacoes_pos$"Mês contra mesmo mês do ano anterior*" > 0, ifelse(tabela_variacoes_pos$"Cod Série" %in% series_contra_ci, "red", "green"), ifelse(tabela_variacoes_pos$"Cod Série" %in% series_contra_ci, "green", "red"))), width = paste0(200, "px", sep = "")),
                         format(round(tabela_variacoes_pos$"Mês contra mesmo mês do ano anterior*", casas_decimais), format = "f", decimal.mark = ","),
                         x ~ icontext(ifelse(tabela_variacoes_pos$"Mês contra mesmo mês do ano anterior*" > 0, "arrow-up", "arrow-down"))),
		
		"Média em 12 meses*" = formatter("span",
                                style = x ~ style(color = ifelse(tabela_variacoes_pos$"Categoria" == "Índices de Preços", NA, ifelse(tabela_variacoes_pos$"Média em 12 meses*" > 0, ifelse(tabela_variacoes_pos$"Cod Série" %in% series_contra_ci, "red", "green"), ifelse(tabela_variacoes_pos$"Cod Série" %in% series_contra_ci, "green", "red")))),
                                format(round(tabela_variacoes_pos$"Média em 12 meses*", casas_decimais), format = "f", decimal.mark = ","),
                                x ~ icontext(ifelse(tabela_variacoes_pos$"Média em 12 meses*" > 0, "arrow-up", "arrow-down"))),
		
		"Mês contra mês imediatamente anterior**" = formatter("span",
                                style = x ~ style(color = ifelse(tabela_variacoes_pos$"Categoria" == "Índices de Preços", NA, ifelse(tabela_variacoes_pos$"Mês contra mês imediatamente anterior**" > 0, ifelse(tabela_variacoes_pos$"Cod Série" %in% series_contra_ci, "red", "green"), ifelse(tabela_variacoes_pos$"Cod Série" %in% series_contra_ci, "green", "red")))),
                                format(round(tabela_variacoes_pos$"Mês contra mês imediatamente anterior**", casas_decimais), format = "f", decimal.mark = ","),
                                x ~ icontext(ifelse(tabela_variacoes_pos$"Mês contra mês imediatamente anterior**" > 0, "arrow-up", "arrow-down"))),
		
		"Média Móvel 3 Meses**" = formatter("span",
                                style = x ~ style(color = ifelse(tabela_variacoes_pos$"Categoria" == "Índices de Preços", NA, ifelse(tabela_variacoes_pos$"Média Móvel 3 Meses**" > 0, ifelse(tabela_variacoes_pos$"Cod Série" %in% series_contra_ci, "red", "green"), ifelse(tabela_variacoes_pos$"Cod Série" %in% series_contra_ci, "green", "red")))),
                                format(round(tabela_variacoes_pos$"Média Móvel 3 Meses**", casas_decimais), format = "f", decimal.mark = ","),
                                x ~ icontext(ifelse(tabela_variacoes_pos$"Média Móvel 3 Meses**" > 0, "arrow-up", "arrow-down"))),
