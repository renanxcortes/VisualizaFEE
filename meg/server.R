...
shinyServer(function(input, output, session) {
  ...
  output$plot_monitor <- renderPlotly({
    ...
    base_aux <- base_transformada %>%
                    filter(year == input_ano, month == input_mes, CategoriaPIB %in% input_categoria, Categoria != "Índices de Preços") %>%
                    na.omit()
...
output$plot_series_teste <- renderPlotly({
	...
	output$tabela_colla_final <- renderFormattable ({
	...
	tabela_collapsible <- tabela_brasileirao_pre %>%
	...
	colnames(tabela_collapsible) <- c("Nome da Série", "Valor Bruto", "Mês contra mesmo mês do ano anterior*", "Média em 12 meses*", "Mês contra mês imediatamente anterior**", "Média Móvel 3 Meses**", "Cod Série", "Categoria")
	
	
	formattable(tabela_collapsible, 
		align = c("l", "c", "r", "r", "r", "r"),
		list(
		...
		"Valor Bruto" = formatter("span",
						 ifelse(tabela_collapsible$"Nome da Série" == "Agronegócio (pessoas ocupadas)", 
						 formatC(tabela_collapsible$"Valor Bruto", digits = 0, big.mark = ".", format = "f", decimal.mark = ","), 
						 formatC(tabela_collapsible$"Valor Bruto", digits = 1, big.mark = ".", format = "f", decimal.mark = ","))
						 ),

		"Mês contra mesmo mês do ano anterior*" = formatter("span",
                         style = x ~ style(color = ifelse(tabela_collapsible$"Categoria" == "Índices de Preços", NA, ifelse(tabela_collapsible$"Mês contra mesmo mês do ano anterior*" > 0, ifelse(tabela_collapsible$"Cod Série" %in% series_contra_ci, "red", "green"), ifelse(tabela_collapsible$"Cod Série" %in% series_contra_ci, "green", "red"))), width = paste0(200, "px", sep = "")),
                         format(round(tabela_collapsible$"Mês contra mesmo mês do ano anterior*", casas_decimais), format = "f", decimal.mark = ","),
                         x ~ icontext(ifelse(tabela_collapsible$"Mês contra mesmo mês do ano anterior*" > 0, "arrow-up", "arrow-down"))),
		"Média em 12 meses*" = formatter("span",
                                style = x ~ style(color = ifelse(tabela_collapsible$"Categoria" == "Índices de Preços", NA, ifelse(tabela_collapsible$"Média em 12 meses*" > 0, ifelse(tabela_collapsible$"Cod Série" %in% series_contra_ci, "red", "green"), ifelse(tabela_collapsible$"Cod Série" %in% series_contra_ci, "green", "red")))),
                                format(round(tabela_collapsible$"Média em 12 meses*", casas_decimais), format = "f", decimal.mark = ","),
                                x ~ icontext(ifelse(tabela_collapsible$"Média em 12 meses*" > 0, "arrow-up", "arrow-down"))),
		"Mês contra mês imediatamente anterior**" = formatter("span",
                                style = x ~ style(color = ifelse(tabela_collapsible$"Categoria" == "Índices de Preços", NA, ifelse(tabela_collapsible$"Mês contra mês imediatamente anterior**" > 0, ifelse(tabela_collapsible$"Cod Série" %in% series_contra_ci, "red", "green"), ifelse(tabela_collapsible$"Cod Série" %in% series_contra_ci, "green", "red")))),
                                format(round(tabela_collapsible$"Mês contra mês imediatamente anterior**", casas_decimais), format = "f", decimal.mark = ","),
                                x ~ icontext(ifelse(tabela_collapsible$"Mês contra mês imediatamente anterior**" > 0, "arrow-up", "arrow-down"))),
		"Média Móvel 3 Meses**" = formatter("span",
                                style = x ~ style(color = ifelse(tabela_collapsible$"Categoria" == "Índices de Preços", NA, ifelse(tabela_collapsible$"Média Móvel 3 Meses**" > 0, ifelse(tabela_collapsible$"Cod Série" %in% series_contra_ci, "red", "green"), ifelse(tabela_collapsible$"Cod Série" %in% series_contra_ci, "green", "red")))),
                                format(round(tabela_collapsible$"Média Móvel 3 Meses**", casas_decimais), format = "f", decimal.mark = ","),
                                x ~ icontext(ifelse(tabela_collapsible$"Média Móvel 3 Meses**" > 0, "arrow-up", "arrow-down"))),
